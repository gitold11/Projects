...
for (let element of usersList) {
    ...
    compName.innerText = `Name: ${element.company.name}`;
    let catchPhrase = document.createElement('li');
    catchPhrase.innerText = `Catch Phrase: ${element.company.catchPhrase}`;
    let bs = document.createElement('li');
    bs.innerText = `BS: ${element.company.bs}`;
    if(element.id %2 === 0){
        user.classList.add('silver');
    }else {
        user.classList.add('corn');
    };
